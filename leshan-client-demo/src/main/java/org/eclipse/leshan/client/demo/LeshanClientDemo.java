...
public class LeshanClientDemo {
    ...
    //Rikard: Launch arguments:
    //-oscore yes -u 10.0.0.1 -lh 10.0.0.3
    //Create subinterface with ifconfig eth0:3 10.0.0.3
    
    //Rikard: Method to add OSCORE context information
    private static void addOSCOREContext(String serverURI) throws OSException {
    	...
    	if(serverURI.contains("localhost")) {
    		serverURI = "coap://127.0.0.1";
    	}
    	db.addContext(serverURI, ctx_A);
    	
    	OSCoreCoapStackFactory.useAsDefault();
    }
    
    //Rikard: Method to add OSCORE context information towards bootstrap server
    private static void addBootstrapOSCOREContext(String serverURI) throws OSException {
    	HashMapCtxDB db = HashMapCtxDB.getInstance();
    	AlgorithmID alg = AlgorithmID.AES_CCM_16_64_128;
    	AlgorithmID kdf = AlgorithmID.HKDF_HMAC_SHA_256;
    ...
    public static void main(final String[] args) throws OSException {
        ...
        options.addOption("oscore", true, //Rikard
                "The path to your file with OSCORE context information.");
        ...
        // Rikard: Read OSCORE context information
        boolean oscore = false;
        if (cl.hasOption("oscore")) {
        	oscore = true;
        	
        	//Rikard: If bootstrapping add the OSCORE Context to the bootstrap server
        	if(cl.hasOption("b") == true) {
        		addBootstrapOSCOREContext(serverURI);
        	} else { //Rikard: Otherwise to the management server
        		addOSCOREContext(serverURI);
        	}
        	
        	LOG.info("Client uses OSCORE.");
        }
